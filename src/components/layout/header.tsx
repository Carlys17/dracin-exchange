  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 border-b border-border bg-bg-deep/90 backdrop-blur-md">
      <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-4">
        ...
        {/* Nav links like ShieldScan */}
        <nav className="hidden md:flex items-center gap-1">
          {["Swap", "Bridge", "Routes"].map((item, i) => (
            <button key={item} className={`rounded-lg px-3 py-1.5 text-[13px] font-medium transition-colors ${i === 0 ? "bg-bg-elevated text-text-primary" : "text-text-muted hover:text-text-secondary hover:bg-bg-card"}`}>
              {item}
            </button>
          ))}
        </nav>

        {/* Wallet — ShieldScan style buttons */}
        <div className="flex items-center gap-2">
          <button className="btn-outline hidden sm:flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-[12px] font-semibold">
